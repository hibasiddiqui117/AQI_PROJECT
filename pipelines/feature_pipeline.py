"""
Feature Pipeline - Fetches AQI data and stores in DagsHub Feature Store
"""
import requests
import pandas as pd
import numpy as np
...
# Import DagsHub feature store
from src.feature_store.dagshub_feature_store import save_features, save_latest, setup_dagshub
...
try:
    ...
    # Add time features for processed data
    ...
    print(f"Features available: {len(df_processed.columns)}")
    
    # Save to DagsHub Feature Store
    print("\nSaving to DagsHub Feature Store...")
    
    try:
        # Save full features
        version = save_features(df_processed, "aqi_features")
        print(f"Features saved to DagsHub - Version: {version}")
        
        # Save latest features
        save_latest(df_processed.tail(100))
        print("Latest features saved to DagsHub")
        
    except Exception as e:
        print(f"DagsHub save warning: {e}")
        print("Data still saved locally")
